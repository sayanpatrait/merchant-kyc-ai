...
public class KycService {
	...
	public String summary() {
		...
		return "Merchants: " + total + ". KYC checks — verified: " + verified + ", pending: " + pending + ", failed: "
				+ failed + ".";
	}
	...
	public CompletableFuture<Void> verifyRemaining() {
		merchantRepository
				.findByPanStatusOrAadhaarStatusOrGstStatus(KycStatus.PENDING, KycStatus.PENDING, KycStatus.PENDING)
	...
	private void verifyIfPending(Merchant merchant, KycType type) {
		...
		// Store reason if failed
		if (result.status() == KycStatus.FAILED && result.reason() != null) {
			merchant.setFailureReason(result.reason());
		}
	}

	public KycService(MerchantRepository merchantRepository, KycProvider kycProvider, OpsEventPublisher opsEvents) {
